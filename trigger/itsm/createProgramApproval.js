...
try {
  const { objectId: releaseProgramApprovalId } = body;

  printLogs(`获取Id为 ${releaseProgramApprovalId} 的投产变更申请单事项数据`);
  // 获取上线计划申请单事项
  const releaseApprovalParse = await apis.getData(false, "Item", {
    objectId: releaseProgramApprovalId,
  });

  const releaseProgramApprovalItem = releaseApprovalParse.toJSON();

  printLogs("投产变更申请单事项数据获取完毕");

  printLogs(
    `获取 ${releaseProgramApprovalId} 投产变更申请单涉及关联的业务需求数据`
  );
  ...
  const {
    ...
    createdBy: releaseProgramApprovalCreatedBy, // 创建人
  } = releaseProgramApprovalItem;

  printLogs(
    `投产变更申请单 ${releaseProgramApprovalId} 中涉及的业务需求列表为`,
  ...
  const createItemRequests = associated_business_requirement?.reduce(
    async (requests, requirementId) => {
      printLogs(
        `查询 ${releaseProgramApprovalId} 投产变更审批单关联的 ${requirementId} 业务需求数据`
      );
        ...
        new Promise(async (resolve, reject) => {
          try {
            ...
            const releaseProgramValues = {
              ...
              demand_leader: releaseProgramApprovalCreatedBy, // 需求牵头人
              online_plan_risk_assessment: [releaseApprovalId], // 上线计划申请单
              associated_product_change_apply_number: ItemCode, // 投产变更申请单事项编号
            };
  ...
  printLogs("开始在不同业务需求下创建投产变更审批单");

  console.log(createItemRequests);

  Promise.all(createItemRequests)
    .then(() => {
